...
export function buildVoiceAssistantSystemInstruction(
  ...
  const lines = [
    ...
    '- When calling submit_user_request or submit_user_hint, put clear English for the coding agent in `text`.',
    '- When calling submit_user_request or submit_user_hint, put the natural user-facing wording in `canonicalUserText` so the UI and conversation language stay aligned.',
    ...
    '- When a controller turn provides controller plan/update text, speak only that message for the user and then stop.',
    '- Never preface controller plan/update text with labels like "Notification", "Update", or "Plan update"; start directly with the message content.',
    '- Controller plan/update turns are not user requests: never call tools, revisit approvals, or continue a previous task during that turn.',
    ...
    '- If a non-Latin transcript is really phonetic English or another transliterated Latin-script utterance, normalize canonicalUserText back into the normal Latin script before handing work off.',
